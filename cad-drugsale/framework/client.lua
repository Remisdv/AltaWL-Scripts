...
if Config.Dispatch == 'custom' then
    function Framework:PoliceAlert()
        ...
        local data = {
            code = '10-38', -- Alert code
            default_priority = 'low', -- Priority
            coords = playerData.coords, -- Coordinates
            job = {'lspd'}, -- Job
            text = "10-38 Action suspicieuse", -- Alert message
            type = 'alerts', -- Alert type
            blip_time = 60, -- Blip display time in seconds
            image = image or nil, -- Image URL
            blip = { -- Blip settings
                sprite = 403,
                colour = 1,
                scale = 1.5,
                flashes = true,
                text = 'Action suspicieuse (possible vente de drogue)'
            }
        }

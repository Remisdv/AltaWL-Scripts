...
local function showSellMenu(ped, item, amt, price)
	...
	if Config.OxMenu then
		lib.registerMenu({
			...
			title = ('%dx de %s pour %d$'):format(amt, Framework:GetItemLabel(item), round(amt * price, 0)),
			position = 'top-right',
			...
			options = {
				{label = 'Accepter l\'offre', args = { type = 'buy', item = item, price = price, amt = amt, tped = ped }},
				{label = 'Refuser l\'offre', args = { type = 'close', tped = ped }},
			}
...
-- 	registerCommand('vente', function()
-- 		AllowedTarget = not AllowedTarget
-- 		if AllowedTarget then
-- 			createTarget()
-- 			Framework:Notify("Vente activé")
-- 		else
-- 			removeTarget()
-- 			Framework:Notify("Vente désactivé")
-- 		end
-- 	end, false)
-- end
